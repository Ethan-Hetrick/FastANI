...
The impact claim for this fork is based on reproducible benchmarking materials included in the repository. On a maintained half-list benchmark using Release-mode builds, the current no-sketch execution path is about 15.3% faster than the original tool overall, with the largest improvement in query mapping. More substantially, prebuilt sketch use changes the cost structure of repeated querying: loading a prebuilt sketch is about 24.1x faster than rebuilding the reference-side data structures in the benchmarked configuration.
...
The larger all-v-all benchmark shows the same direction at fuller scale. For `genome-list.txt` queried against itself, the original tool required `1076.02s`, while the new workflow required `22.52s` for sketch creation and `483.99s` for sketch-backed querying, for a total of `506.51s` and an overall speedup of about `2.13x`. After correcting sketch metadata handling in the release-mode sketch path, the old and new all-v-all outputs reported the same number of comparisons (`1,310,658` rows), which helps separate true performance gains from output-format or filtering artifacts.
