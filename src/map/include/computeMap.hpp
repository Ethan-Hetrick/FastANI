  ...
  while ((len = kseq_read(seq)) >= 0)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < fragmentCount; i++)
      {
        ...
        std::sort(frag.minimizerTableQuery.begin(), frag.minimizerTableQuery.end(),
                  MinimizerInfo::lessByHash);

        auto uniqEndIter =
          std::unique(frag.minimizerTableQuery.begin(), frag.minimizerTableQuery.end(),
...
public:
   ...
   */
  Map(const skch::Parameters &p, const skch::Sketch &refsketch, uint64_t &totalQueryFragments,
      int queryno, PostProcessResultsFn_t f = nullptr)
...
private:
...
#ifdef DEBUG
          std::cerr << "WARNING, skch::Map::mapQuery, read is not long enough for mapping"
                    << std::endl;
#endif
        }
        else
          ...
          for (int i = 0; i < fragmentCount; i++)
          {
            ...
            if (param.visualize)
            {
              if (i != fragmentCount - 1)
                ...
                metadata.push_back(
                  ContigInfo{seq->name.s, param.minReadLength + (len % param.minReadLength)});
            }
  ...
  inline void mapSingleQuerySeq(Q_Info &Q, std::vector<L1_candidateLocus_t> &l1Mappings,
                                MappingResultsVector_t &l2Mappings, std::ofstream &outstrm)
  {
...
#if ENABLE_TIME_PROFILE_L1_L2
    {
      ...
      std::cerr << Q.kseq->name.s << " " << Q.kseq->seq.l << " " << countL1Candidates << " "
                << timeSpentL1.count() << " " << timeSpentL2.count() << " "
   ...
   */
  template <typename Q_Info, typename Vec> void doL1Mapping(Q_Info &Q, Vec &l1Mappings)
  {
    ...
    CommonFunc::addMinimizers(Q.minimizerTableQuery, Q.kseq, param.kmerSize, param.windowSize,
                              param.alphabetSize);

#ifdef DEBUG
    std::cerr << "INFO, skch::Map:doL1Mapping, read id " << Q.seqCounter
              << ", minimizer count = " << Q.minimizerTableQuery.size() << "\n";
#endif

    /// 2. Find the hits in the reference, pick 's' unique minimizers as seeds,

    std::sort(Q.minimizerTableQuery.begin(), Q.minimizerTableQuery.end(),
              MinimizerInfo::lessByHash);

    // note : unique preserves the original relative order of elements
    auto uniqEndIter = std::unique(Q.minimizerTableQuery.begin(), Q.minimizerTableQuery.end(),
                                   MinimizerInfo::equalityByHash);
    ...
    int minimumHits =
      Stat::estimateMinimumHitsRelaxed(Q.sketchSize, param.kmerSize, param.percentageIdentity);
  ...
  void computeL1CandidateRegions(Q_Info &Q, Vec1 &seedHitsL1, int minimumHits, Vec2 &l1Mappings)
  {
    ...
    for (auto it = seedHitsL1.begin(); it != seedHitsL1.end(); it++)
    {
      if (std::distance(it, seedHitsL1.end()) >= minimumHits)
      {
        ...
        // NOTE: hits may span more than a read length for a valid match, as we keep window
        // positions
        //       for each minimizer
        if (it2->seqId == it->seqId && it2->wpos - it->wpos < Q.kseq->seq.l)
        {
          // Save <1st pos --- 2nd pos>
          L1_candidateLocus_t candidate{
            it->seqId, std::max(0, it2->wpos - offset_t(Q.kseq->seq.l) + 1), it->wpos};
  ...
  bool doL2Mapping(Q_Info &Q, VecIn &l1Mappings, VecOut &l2Mappings)
  {
    ...
    /// 2. Walk the read over the candidate regions and compute the jaccard similarity with minimum
    /// s sketches
    for (auto &candidateLocus : l1Mappings)
    {
      ...
      float mash_dist_lower_bound =
        Stat::md_lower_bound(mash_dist, Q.sketchSize, param.kmerSize, 0.9);
  ...
  void computeL2MappedRegions(Q_Info &Q, L1_candidateLocus_t &candidateLocus, L2_mapLocus_t &l2_out)
  {
    ...
    MIIter_t firstSuperWindowRangeStart =
      this->refSketch.searchIndex(candidateLocus.seqId, candidateLocus.rangeStartPos);
    ...
    MIIter_t firstSuperWindowRangeEnd = this->refSketch.searchIndex(
      candidateLocus.seqId, firstSuperWindowRangeStart->wpos + countMinimizerWindows);

    // Look up L1 candidate's end in the index
    MIIter_t lastSuperWindowRangeEnd =
      this->refSketch.searchIndex(candidateLocus.seqId, candidateLocus.rangeEndPos + Q.kseq->seq.l);
