...
REPEATS="${1:-1}"
THREADS="${2:-8}"
RAW_CSV="${WORK_DIR}/old_vs_new_20v_all_runs.csv"
...
if [[ ! -f "$QUERY_LIST" || ! -f "$REF_LIST" ]]; then
  echo "Missing query/reference lists. Run prepare_apicomplexa_inputs.sh first." >&2
  exit 1
fi

cat > "$RAW_CSV" <<'EOF'
variant,replicate,binary_path,threads,query_list,reference_list,output_path,output_rows,internal_db_phase_label,internal_db_phase_sec,internal_query_map_sec,internal_post_map_sec,wall_sec,user_cpu_sec,system_cpu_sec,cpu_percent,max_rss_kb,fs_inputs_blocks,fs_outputs_blocks,major_page_faults,minor_page_faults,voluntary_ctx_switches,involuntary_ctx_switches
EOF
...
append_csv_row() {
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$variant" "$rep" "$binary_path" "$THREADS" "$QUERY_LIST" "$REF_LIST" "$output_path" "$output_rows" \
...
run_case() {
  ...
  for rep in $(seq 1 "$REPEATS"); do
    prefix="${WORK_DIR}/${variant}_rep${rep}"
    output_path="${prefix}.out"
    stdout_path="${prefix}.stdout"
    stderr_path="${prefix}.stderr"

    rm -f "$output_path" "$stdout_path" "$stderr_path"
    /usr/bin/time -v "$binary" --ql "$QUERY_LIST" --rl "$REF_LIST" -t "$THREADS" -o "$output_path" > "$stdout_path" 2> "$stderr_path"
    append_csv_row "$variant" "$rep" "$binary" "$output_path" "$stderr_path"
  done
}
